...
if(isset($_POST['latest_news_time'])){
	$sql = "SELECT * FROM events WHERE date1 > '".$_POST['latest_news_time']."' ORDER BY date1 DESC";
	...
	<li id="<?php echo $item['date1'] ?>">
		<span class="feedtext"><?php echo "<b>Crime occured at:</b> <br>".$item['address']." on <b>Date </b> ".$item['date1'] ?></span>
	</li>
	<?php
	exit;
}
...
**/
$sql = "SELECT * FROM events ORDER BY date1 DESC LIMIT 0, 10";
if(isset($_POST['last_time'])){
	$sql = "SELECT * FROM events WHERE date1 < '".$_POST['last_time']."' ORDER BY date1 DESC LIMIT 0, 10";
}
$resource = mysql_query($sql);
$news = array();
